...
export default function LeadForm() {
  ...
  // Засекаем в эффекте, а не при рендере: Date.now() — нечистый вызов, при повторном
  // рендере (в том числе при отрисовке в двух проходах) он дал бы разные значения.
  // Момент готовности формы к вводу — как раз монтирование, так что это ещё и точнее.
        ...
        <div data-reveal className="mt-[14px] flex flex-col gap-[5px] md:mt-[40px] md:flex-row md:gap-[30px]">
          ...
          <div className="rounded-[15px] border border-[#eee] bg-[#fcfcfc] px-[15px] py-[24px] md:flex-1 md:px-[36px] md:py-[46px]">
            {status === "sent" ? (
              ...
              // void: onSubmit асинхронный, а обработчик события обязан вернуть void. Без
              // этого промис остаётся без обработчика отказа — сбой отправки всплыл бы как
              // unhandled rejection мимо нашего catch
