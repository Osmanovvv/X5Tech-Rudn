  ...
  },
  {
    ...
    text: (
      <>
        ...
        <span className="font-medium">X5 Tech</span>, стажировки <br />и карьерные треки
      </>
    ),
  },
  {
...
// «X5 Tech» внутри текста карточек в макете набран medium (карточки 1 и 3). Мобильные тексты
// хранятся строками ради жёстких переносов, поэтому выделяем бренд разбором строки.
function withBrand(text: string) {
  return text.split(/(X5 Tech)/g).map((part, i) =>
    part === "X5 Tech" ? (
...
function CardIllustration({ k, mobile }: { k: string; mobile?: boolean }) {
  ...
  switch (k) {
    case "industry":
      return (
        <>
          <span className="absolute" style={mobile ? { left: 191, top: 27 } : { left: 186, top: 62 }}>
    ...
    <section aria-label="Программа даёт" id="programma">
      ...
      <div className="canvas-320 relative overflow-hidden bg-white md:hidden">
        ...
        <div data-reveal className="mx-[15px] mt-[15px] flex flex-col gap-[15px] pb-[30px]">
          {/* Переносы строк — дословно из макета (в мобильном они те же, что на десктопе:
              сверено по эталонному рендеру section-03-programma-daet.png). Раньше здесь
              стояла своя разбивка, не совпадавшая с макетом.
              Кегль текста 11→12 по просьбе заказчика (docs/deviations.md D10). Выше нельзя:
              у первой карточки строка «Обучение вместе с X5 Tech –» при 13px занимает 191px
              при 171 свободных до 3D-объекта — то есть налезает на него. Заголовок остаётся
              макетным 12: на 13px «Топовую техническую инфраструктуру» требует 264px при 250
              доступных и переносится на две строки, чего в макете нет. */}
          {[
            { c: CARDS[0], mtext: "Обучение вместе с X5 Tech –\nэто не экскурсии в офис раз\nв год, а работа с реальными\nзадачами, которые\nподготовят тебя к карьере.", h: 129 },
            { c: CARDS[1], mtext: "С первого курса работаешь\nнад реальными задачами,\nа не абстрактными\nупражнениями", h: 129 },
            { c: CARDS[2], mtext: "Прямой контакт\nс ML-инженерами\nX5 Tech, стажировки\nи карьерные треки", h: 129 },
            { c: CARDS[3], mtext: "Технологичные аудитории,\nAI-лаборатории\nи пространства\nдля командной работы", h: 129 },
            ...
            <div
              ...
              style={{ minHeight: h }}
            >
              <p className="relative z-[1] px-[20px] pt-[13px] text-[12px] font-bold leading-[14px] text-ink">
                {c.title}
              </p>
              {/* Нижний отступ 14, а не 16: рамка 1.13px с обеих сторон входит в высоту
                  (border-box), и на самой длинной карточке 16 давали 131 вместо макетных 129 */}
              <p className="relative z-[1] ml-[20px] mt-[16px] whitespace-pre-line pb-[14px] text-[12px] leading-[14px] text-ink/85">
                {withBrand(mtext)}
              </p>
