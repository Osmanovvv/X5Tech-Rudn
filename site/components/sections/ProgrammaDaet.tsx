    ...
    <section aria-label="Программа даёт" id="programma">
      ...
      <div className="canvas-320 relative overflow-hidden bg-white md:hidden">
        ...
        <div data-reveal className="mx-[15px] mt-[15px] flex flex-col gap-[15px] pb-[30px]">
          {/* Кегли подняты по просьбе заказчика: заголовок 12→14, текст 11→13 (макетные —
              12 и 11, см. docs/deviations.md D10). Из-за этого высота карточки больше не
              жёсткая, а минимальная, и текст лежит в потоке: у первой карточки текст самый
              длинный и на крупном кегле перестал помещаться в макетные 129px. Иллюстрации
              остаются абсолютными и привязаны к низу — растущая карточка их не ломает. */}
          {[
            { c: CARDS[0], mtext: "Обучение вместе с X5 Tech – это не экскурсии в офис раз в год, а работа с реальными задачами, которые подготовят тебя к карьере.", h: 129, tW: 171 },
            { c: CARDS[1], mtext: "С первого курса работаешь над реальными задачами, а не абстрактными упражнениями", h: 129, tW: 184 },
            { c: CARDS[2], mtext: "Прямой контакт с ML-инженерами X5 Tech, стажировки и карьерные треки", h: 129, tW: 178 },
            { c: CARDS[3], mtext: "Технологичные аудитории, AI-лаборатории и пространства для командной работы", h: 129, tW: 201 },
            { c: CARDS[4], mtext: "Вычислительные кластеры, GPU-серверы и облачные платформы для обучения моделей", h: 126, tW: 217 },
          ].map(({ c, mtext, h, tW }) => (
            <div
              key={c.key}
              ...
              style={{ minHeight: h }}
            >
              <p className="relative z-[1] px-[20px] pt-[13px] text-[14px] font-bold text-ink">
                {c.title}
              </p>
              {/* Переносы больше не прошиты вручную: они были подобраны под 11px и на
                  крупном кегле рвали абзац посреди фразы, оставляя куцые строки в треть
                  ширины. Текст заливает свой столбец сам — ширина столбца упирается в
                  левый край иллюстрации, поэтому наезда не будет. */}
              <p
                className="relative z-[1] ml-[20px] mt-[13px] pb-[16px] text-[13px] leading-[16px] text-ink/85"
                style={{ width: tW }}
              >
                {mtext}
              </p>
