...
## Замена шрифта на X5 Sans

Сейчас сайт набран Inter — файлы фирменного X5 Sans пока не переданы. Когда они появятся:

1. Положите `X5Sans-Regular.woff2`, `X5Sans-Medium.woff2`, `X5Sans-Bold.woff2` в `app/fonts/`.
2. В `app/fonts.ts` замените три пути в `fontSans.src` (веса 400/500/700 остаются те же).
3. `node scripts/make-og.mjs` — карточка соцсетей перерисуется новым шрифтом.
4. `node scripts/qa-figma.mjs` — сквозная сверка с макетами. **Это обязательный шаг:** метрики
   шрифтов различаются, поэтому переносы строк и высоты блоков поплывут, и часть секций
   потребует правки отступов.
...
- Шрифт: используется Inter вместо фирменного X5 Sans — файлы не переданы. Порядок замены
  описан выше; из-за разницы метрик это единственное, что мешает полному совпадению с макетом.
