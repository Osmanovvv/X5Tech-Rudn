# Сайт «Факультет искусственного интеллекта РУДН × X5 Tech»

Лендинг программы бакалавриата, раздел новостей, правовые страницы, веб-админка и приём заявок
с формы. Технологии: Next.js 16 (App Router), React, TypeScript, Tailwind CSS v4.

---

## Что нужно для запуска

- **Node.js 20 или новее** (проверить: `node -v`)
- Ничего больше: базы данных и сторонние сервисы не требуются — данные хранятся в файлах.

---

## Два режима работы

Проект собирается в одном из двух режимов. Выбирается командой сборки.

| | `npm run build` (по умолчанию) | `npm run build:static` |
|---|---|---|
| Что получается | Node-приложение в `.next/standalone` | Папка `out/` со статикой |
| Что нужно на сервере | Node.js | любой веб-сервер (nginx, Apache) |
| Админка `/admin` | **есть** | нет |
| Приём заявок с формы | **есть** | нет (форма честно сообщит, что отправка не подключена) |
| Новости меняются | через админку, **без пересборки** | правкой файла + пересборка |

Основной вариант поставки — **первый** (Node). Второй оставлен как запасной, если сервер с Node
окажется недоступен.

---

## Быстрый старт (разработка)

```bash
npm ci
npm run dev
```

Сайт откроется на `http://localhost:3000`. Админка — `http://localhost:3000/admin`
(нужен `.env`, см. ниже).

---

## Развёртывание на сервере (основной вариант)

### 1. Подготовить каталог данных

```bash
sudo mkdir -p /var/lib/rudn-ai/data
sudo chown -R www-data:www-data /var/lib/rudn-ai
```

Здесь будут храниться новости, заявки и загруженные обложки. **Каталог должен находиться вне
папки с приложением** — иначе следующий деплой сотрёт данные. Именно его нужно бэкапить.

### 2. Создать `.env`

Скопируйте `.env.example` в `.env` и заполните. Обязательный минимум:

```bash
cp .env.example .env
npm run admin:secret            # выведет строку SESSION_SECRET=...
npm run admin:hash -- "ваш-длинный-пароль"   # выведет строку ADMIN_PASSWORD_HASH=...
```

Обе строки вставьте в `.env`, задайте `ADMIN_USERNAME` и `DATA_DIR`.
Открытый пароль нигде не сохраняется — только его хэш.

### 3. Собрать и запустить

```bash
npm ci
npm run build
PORT=3000 HOSTNAME=127.0.0.1 node .next/standalone/server.js
```

Сборка сама копирует `.env` рядом с сервером — остальные настройки (доступ в админку, `DATA_DIR`,
Telegram) подхватятся из него.

> **`PORT` и `HOSTNAME` — исключение:** их сервер читает раньше, чем загружает `.env`, поэтому
> задавайте их в командной строке (как выше) или через systemd (следующий шаг). Иначе сервер
> слушает `0.0.0.0:3000` и оказывается доступен снаружи в обход nginx и TLS.

> **Перед любой пересборкой останавливайте работающий сервер.** Сборка перезаписывает каталог
> `.next`, из которого запущено приложение: на Windows система просто не даст этого сделать,
> на Linux сборка пройдёт, но работающий процесс останется без файлов и начнёт отдавать ошибки.
> Правильный порядок: `systemctl stop rudn-ai` → `npm run build` → `systemctl start rudn-ai`.

### 4. Автозапуск (systemd)

`/etc/systemd/system/rudn-ai.service`:

```ini
[Unit]
Description=Сайт факультета ИИ РУДН
After=network.target

[Service]
Type=simple
User=www-data
WorkingDirectory=/var/www/rudn-ai
EnvironmentFile=/var/www/rudn-ai/.env
ExecStart=/usr/bin/node .next/standalone/server.js
Restart=always
RestartSec=5

[Install]
WantedBy=multi-user.target
```

```bash
sudo systemctl daemon-reload
sudo systemctl enable --now rudn-ai
sudo systemctl status rudn-ai
```

### 5. nginx

```nginx
server {
    listen 443 ssl http2;
    server_name ai.rudn.ru;

    ssl_certificate     /etc/letsencrypt/live/ai.rudn.ru/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/ai.rudn.ru/privkey.pem;

    # Загрузка обложек новостей — до 8 МБ
    client_max_body_size 10m;

    # Сжатие обязательно. Без него страница весит ~780 КБ вместо ~130 КБ, и это
    # примерно 2 секунды и 30 баллов Lighthouse на мобильном соединении.
    gzip              on;
    gzip_comp_level   6;
    gzip_min_length   1024;
    gzip_proxied      any;
    gzip_vary         on;
    gzip_types        text/plain text/css application/javascript application/json
                      application/xml image/svg+xml;

    # Файлы Next с хешем в имени неизменяемы — кэшируем навсегда
    location /_next/static/ {
        proxy_pass  http://127.0.0.1:3000;
        add_header  Cache-Control "public, max-age=31536000, immutable";
    }

    # Загруженные обложки: в имени файла метка времени, новая картинка получает новый
    # адрес — вечный кэш безопасен. Отдельный блок нужен, чтобы правило ниже не сбросило
    # им заголовок, который ставит само приложение.
    location /api/uploads/ {
        proxy_pass         http://127.0.0.1:3000;
        proxy_http_version 1.1;
        proxy_set_header   Host              $host;
        proxy_set_header   X-Forwarded-Proto $scheme;
    }

    location / {
        proxy_pass         http://127.0.0.1:3000;
        proxy_http_version 1.1;
        proxy_set_header   Host              $host;
        proxy_set_header   X-Real-IP         $remote_addr;
        proxy_set_header   X-Forwarded-For   $proxy_add_x_forwarded_for;
        proxy_set_header   X-Forwarded-Proto $scheme;

        # ОБЯЗАТЕЛЬНО. Next отдаёт готовым страницам «Cache-Control: s-maxage=31536000»
        # без max-age. Общий кэш (CDN, а перед доменом РУДН стоит DDoS-Guard) понимает
        # это как «держать год»: правки новостей просто не доедут до посетителей.
        # Браузер без max-age тоже кэширует по своему усмотрению — на телефоне это
        # выглядит как «сайт не обновился». Заставляем перепроверять разметку.
        proxy_hide_header  Cache-Control;
        add_header         Cache-Control "public, max-age=0, must-revalidate" always;
    }
}

server {
    listen 80;
    server_name ai.rudn.ru;
    return 301 https://$host$request_uri;
}
```

Заголовки `X-Real-IP` / `X-Forwarded-For` обязательны: по ним работает защита от подбора пароля
и ограничение частоты заявок.

### 6. Обновление сайта

```bash
# остановить, обновить файлы приложения, затем:
npm ci
npm run build
sudo systemctl restart rudn-ai
```

Каталог `DATA_DIR` при этом не трогается — новости и заявки сохраняются.

> **Собирайте на сервере, а не на стороне.** `npm run build` заранее отрисовывает страницы
> новостей и главную, подставляя данные из того `DATA_DIR`, который виден **машине сборки**.
> Если собрать у себя и залить готовую сборку, посетитель до первой правки в админке будет
> видеть данные сборочной машины, а не сервера: файл в `DATA_DIR` на сервере при этом
> правильный, но страницы уже отрисованы. Любое сохранение в админке приводит их в порядок —
> она пересобирает затронутые страницы сама.

---

## Развёртывание статикой (запасной вариант)

```bash
npm ci
npm run build:static
```

Полученную папку `out/` разместить на веб-сервере. Настроить страницу ошибки:

```nginx
error_page 404 /404.html;
```

В этом режиме нет админки и приёма заявок. Чтобы форма всё же отправляла заявки, укажите адрес
внешнего обработчика в `content/site.json` → `leadEndpoint`.
Новости в этом режиме правятся в `content/news.json` с последующей пересборкой.

---

## Админка

Адрес: `/admin`. Вход — логин и пароль из `.env`.

- **Новости** — добавление, редактирование, удаление; загрузка обложки (JPEG, PNG, WebP, AVIF,
  шире 600 px, до 8 МБ — сжатие и подгонка размеров происходят автоматически). Изменения видны
  на сайте сразу, пересборка не нужна.
  Адрес новости (`/news/…`) задаётся при создании по заголовку и потом не меняется — чтобы уже
  разосланные ссылки продолжали работать.
- **Контент** — состав трёх секций главной страницы, тоже без пересборки:
  - **Программа обучения** — курсы, их подписи и дисциплины (порядок меняется стрелками),
    лавандовая плашка, карточка статистики, фото курсов;
  - **Преподаватели и эксперты** — состав, фотографии и кадрирование, подписи секции;
  - **Как поступить** — стоимость, три показателя, этапы приёма по бюджету и договору,
    таблица вступительных экзаменов.

  У каждой секции есть кнопка «вернуть как в макете» — она заменяет содержимое исходным,
  с которым сайт был сдан, и не трогает остальные разделы.

  Поля с пометкой «на телефоне» нужны там, где в макете на узкой ширине текст короче или
  разбит иначе; если оставить их пустыми, возьмётся обычный вариант. Переносы строк в полях
  сохраняются как есть — на них держится разбивка из макета.

### Картинки внутри текста новости

Абзацы разделяются пустой строкой. Картинка ставится **отдельной строкой**:

```
![Что на фото](upload:den-otkrytyh-dverej-mfk3x1)
```

Две такие записи в **одной** строке встанут рядом, в ряд:

```
![Слева](upload:foto-a) ![Справа](upload:foto-b)
```

Чтобы получить код картинки: выберите файлы в поле «Картинки в текст» и сохраните новость —
строки `![](upload:…)` допишутся в конец текста. Откройте новость снова и перенесите их между
нужными абзацами. Удалённая из текста строка удаляет и сам файл, мусор в каталоге не копится.
- **Заявки** — список заявок с формы и выгрузка в CSV (открывается в Excel).

Сессия администратора живёт 8 часов. Смена `SESSION_SECRET` в `.env` мгновенно завершает все
сессии — так можно «разлогинить всех», если пароль скомпрометирован.

---

## Уведомления о заявках в Telegram

Необязательно. Без настройки заявки просто сохраняются и видны в админке.

1. Создайте бота у **@BotFather**, получите токен.
2. **Обязательно напишите боту сообщение** (откройте его в Telegram и нажмите «Start») — или,
   если уведомления идут в группу, добавьте бота в неё.
   Telegram запрещает ботам писать первыми: без этого шага отправка вернёт `chat not found`,
   даже если токен и `chat_id` указаны верно. Это самая частая причина «уведомления не приходят».
3. Узнайте `chat_id`: откройте `https://api.telegram.org/bot<ТОКЕН>/getUpdates` и найдите
   `result[].chat.id` (у групп id отрицательный).
4. Впишите `TELEGRAM_BOT_TOKEN` и `TELEGRAM_CHAT_ID` в `.env`, перезапустите приложение.
5. Проверьте: отправьте заявку через форму на сайте. Если сообщение не пришло — посмотрите
   журнал приложения, там будет причина (`journalctl -u rudn-ai -n 50`).

Если `api.telegram.org` недоступен с сервера, укажите адрес своего прокси Bot API в
`TELEGRAM_API_BASE`.

Недоступность Telegram никогда не приводит к потере заявки: она сначала сохраняется, и только
потом отправляется уведомление.

---

## Резервное копирование

Копировать нужно **только каталог `DATA_DIR`** — в нём всё, что нельзя восстановить пересборкой:

```
$DATA_DIR/news.json      — новости
$DATA_DIR/leads.jsonl    — заявки с сайта (персональные данные)
$DATA_DIR/uploads/       — загруженные обложки
```

Пример ежедневной копии:

```bash
tar czf /backup/rudn-ai-$(date +%F).tar.gz -C /var/lib/rudn-ai data
```

> **Персональные данные.** `leads.jsonl` содержит имена, телефоны и адреса почты абитуриентов.
> Ограничьте доступ к каталогу и к резервным копиям, храните их не дольше, чем требуется.

---

## Размещение в подпапке домена

Если сайт живёт не в корне (например, `https://rudn.ru/ai`), укажите путь и пересоберите:

```bash
NEXT_PUBLIC_BASE_PATH=/ai npm run build
```

---

## Правка контента без админки

Тексты страниц лежат в `content/` (JSON, редактируются в любом текстовом редакторе):

| Файл | Что содержит |
|---|---|
| `site.json` | контакты, адрес, ссылки футера, домен, счётчик Метрики |
| `program.json` | программа обучения: курсы и дисциплины |
| `admission.json` | поступление: цены, сроки, экзамены |
| `teachers.json` | преподаватели |
| `legal.json` | список правовых страниц |
| `legal-bodies.json` | тексты правовых документов |
| `news.json` | демо-новости (в Node-режиме новости берутся из `DATA_DIR`) |

После правки — пересборка (`npm run build`) и перезапуск.

> В Node-режиме `program.json`, `teachers.json` и `admission.json` работают так же, как
> `news.json`: файл в репозитории — исходное содержимое, а после первой правки в админке
> источником становится одноимённый файл в `DATA_DIR`. Он переживает пересборку и деплой,
> поэтому правку файла в репозитории сайт после этого уже не увидит — меняйте через админку
> либо тот же файл в `DATA_DIR`. Кнопка «вернуть как в макете» возвращает содержимое из
> репозитория. В статическом режиме админки нет, и читается только репозиторий.

> Файлы сохраняйте **в кодировке UTF-8 без BOM**. Блокнот Windows добавляет BOM — используйте
> Notepad++, VS Code или аналог. Если файл не удалось прочитать, приложение напишет об этом
> в журнал и продолжит работу на прежних данных.

---

## Что настраивается перед публикацией

> ### ⚠️ Домен — обязательно, иначе сайт закрыт от поисковиков
>
> Пока в `content/site.json` стоит домен-заглушка `ai-rudn.example`, сборка намеренно отдаёт
> `robots.txt` с `Disallow: /` и ставит страницам `noindex`. Так сделано специально: с чужим
> доменом в canonical и карте сайта индексация всё равно была бы вредна. Сборка предупреждает
> об этом в консоли строкой `[SEO] siteUrl не задан`.
>
> Задайте настоящий домен одним из двух способов и пересоберите:
>
> ```bash
> NEXT_PUBLIC_SITE_URL=https://ai.rudn.ru npm run build
> ```
>
> либо впишите его в `content/site.json` → `siteUrl`. После этого `robots.txt` открывает сайт,
> появляется `sitemap.xml`, а canonical и Open Graph начинают указывать на реальные адреса.

| Где | Что |
|---|---|
| `content/site.json` → `siteUrl` | итоговый домен — см. предупреждение выше |
| `content/site.json` → `metrikaId` | идентификатор Яндекс.Метрики (счётчик подключается отложенно, чтобы не проседали баллы скорости). Вебвизор, карта кликов и точный показатель отказов уже включены. Учтите: с подключённым счётчиком Lighthouse снижает раздел «Лучшие практики» примерно до 79 — он засчитывает сторонние куки Метрики; это цена любого счётчика, а не дефект сайта |
| `content/site.json` → `yandexVerification` | код подтверждения прав в Яндекс.Вебмастере |
| `content/legal.json` + страницы `/docs/*` | тексты правовых документов (сейчас заглушки, закрыты `noindex`) |

### Что сайт отдаёт поисковикам

| Адрес | Что это |
|---|---|
| `/robots.txt` | генерируется из домена; `/admin` и `/api/` закрыты всегда |
| `/sitemap.xml` | главная, список новостей и каждая новость; даты берутся из самих новостей |
| `/og.jpg` | картинка 1200×630 для ссылок в соцсетях и мессенджерах |

Карточку соцсетей и иконки можно перерисовать без графического редактора:

```bash
node scripts/make-og.mjs      # public/og.jpg — рисуется в браузере шрифтами сайта
node scripts/make-icons.mjs   # favicon.ico и apple-icon.png из app/icon.svg
```

---

## Шрифт

Сайт набран фирменным **X5 Sans** (ParaType для X5 Group), начертания Regular / Medium / Bold —
файлы лежат в `app/fonts/`. Если понадобится заменить набор:

1. Положите новые `X5Sans-*.woff2` в `app/fonts/` (пути прописаны в `app/fonts.ts`).
2. `node scripts/make-og.mjs` — карточка соцсетей перерисуется новым шрифтом.
3. `node scripts/qa-figma.mjs` — сквозная сверка с макетами.

---

## Замечание по разработке

Если после правки вёрстка «поехала» без видимой причины — Tailwind в dev-режиме иногда не
подхватывает НОВЫЙ произвольный класс: он попадает в разметку, а правило в CSS не появляется,
и стиль молча не применяется. Лечится так:

```bash
rm -rf .next/cache .next/dev
```

После этого перезапустите `npm run dev`. На продакшн-сборку это не влияет.

---

## Известные ограничения

- Правовые страницы `/docs/*` — заглушки: тексты предоставляет университет.
- Шрифт: подключён фирменный X5 Sans (Regular / Medium / Bold).
- Приложение рассчитано на один экземпляр процесса. Данные — файлы, поэтому запуск нескольких
  копий на одном каталоге не предусмотрен.

---

## Проверки качества

### Быстрая проверка кода — без сервера и браузера

```bash
npm run check
```

Это `lint` + `typecheck` + `test` разом. Гонять перед каждым коммитом.

| Команда | Что делает |
|---|---|
| `npm run lint` | ESLint с типизированными правилами: плавающие промисы, приведения типов, мёртвый код |
| `npm run typecheck` | `tsc --noEmit` по всему проекту |
| `npm test` | модульные тесты (`node:test`): подпись сессии, хэш пароля, разбор текста новости, чтение формы |

Правило `no-img-element` отключено намеренно: `next/image` требует рантайм-оптимизатора,
которого нет в статическом режиме сборки, и подставляет собственную обёртку — то есть меняет
разметку и ломает пиксельную сверку с макетом. Размеры и `srcset` готовит `optimize-images.mjs`.

### Сверка с макетом и живые проверки

Запускаются из папки `site`. Часть работает по dev-серверу (`npm run dev`), часть — по
собранной статике (`npm run build:static`). Проверка админки МЕНЯЕТ данные: создаёт новость и
удаляет её же, поэтому гоняйте её на тестовом `DATA_DIR` (проще всего — временный `.env.local`).

| Команда | Что проверяет |
|---|---|
| `node scripts/qa-figma.mjs` | сквозная сверка всех секций с макетами на 1200 и 320 |
| `node scripts/qa-bands.mjs` | построчная диагностика мобильной: что именно съехало |
| `node scripts/qa-a11y.mjs` | доступность и горизонтальный скролл, 5 страниц × 2 ширины |
| `node scripts/qa-widths.mjs` | масштабирование на 12 ширинах от 320 до 1920 |
| `node scripts/qa-static.mjs` | собранная папка `out/` на обычном сервере: все адреса, 404, служебные файлы |
| `node scripts/qa-lighthouse.mjs` | Lighthouse по `out/` со сжатием — как на боевом nginx |
| `node scripts/qa-retina.mjs` | хватает ли разрешения картинок для экранов двойной плотности |
| `ADMIN_PASS="…" node scripts/qa-admin.mjs` | админка целиком: вход, создание, правка, удаление, защита разделов, выход |
