...
- [x] **Lighthouse** (`scripts/qa-lighthouse.mjs`, замер по собранной статике со сжатием): Perf 91–100, A11y 96–100, Best 96–100, CLS 0.000 везде. Цели плана выполнены. SEO 69 — это наша защита от индексации домена-заглушки, с реальным доменом 100. Таблица и найденные по ходу дефекты — в `docs/budget.md`
- [x] **Ретина** (`scripts/qa-retina.mjs`): из 14 картинок без двукратного запаса шесть пересобраны в бо́льшем разрешении, четыре упираются в разрешение исходника — перечислены в `docs/budget.md`

### Task 6.1 (частично): прогон собранной статики ✅ 2026-07-27
- [x] `scripts/qa-static.mjs` — `out/` поднимается обычным сервером **без SPA-фолбэков**, обходятся все адреса из sitemap плюс все внутренние ссылки со страниц (17 адресов), проверяются 404 с правильным кодом, адрес без слеша и служебные файлы. Осталось от Task 6.1: чистая сборка с нуля по README и финальный архив
