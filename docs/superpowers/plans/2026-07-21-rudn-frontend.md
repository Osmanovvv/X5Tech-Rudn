...
- [ ] `ScrollReveal` — один `IntersectionObserver` на всю страницу (не по обсерверу на элемент):
      `threshold: 0.18` (спека: 15–20 % видимости), после срабатывания `unobserve` — **каждый блок
      проигрывается один раз**, повторно при скролле не запускается
- [ ] Разметка: секции помечаются `data-reveal` (и `data-reveal-group` для волн), обсервер вешает
      класс `is-in`. CSS: `[data-reveal]{opacity:0;translate:0 16px}` включается **только** внутри
      `@media (prefers-reduced-motion: no-preference)` и только когда на `<html>` есть класс `js`
      (ставится тем же островом) — так базовое состояние остаётся видимым
...
- [ ] **01 Первый экран.** Заголовок и текст: `opacity` 0→1 + снизу 16–20 px, 450–550 мс. Фото
      студентов — на 80–120 мс позже, `scale` 0.98→1. Старт **после загрузки шрифтов и главного
      фото** (`document.fonts.ready` + `decode()` LCP-изображения), иначе анимация конкурирует с LCP.
      3D-формы — довернуть **один раз** при загрузке, без бесконечного плавания
...
**Открытые вопросы дизайнеру (ответ нужен до Task 3.2):**
1. **Лаймовые marquee-ленты** «РУДН × X5 TECH • БАКАЛАВРИАТ ИИ» (секции 02 и 03) в спеке не
   упомянуты, при этом действует правило «без постоянного движения». Бегущая строка или статика?
   **Дефолт при молчании: статика** (буквальное следование правилу).
2. **Грант-полоса** с гербом Аналитического центра — попадает под «логотипы партнёров не
   анимировать» или это обычный блок с fade-up? **Дефолт: не анимировать.**
