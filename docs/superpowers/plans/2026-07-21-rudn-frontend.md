...
- **Важно (установлено 2026-07-27):** эталонные рендеры Figma отрисованы НЕ фирменным шрифтом. В файле X5 Sans стоит на 805 узлах, но аккаунту он недоступен, и рендерер подменяет его на Inter — в сетевых запросах редактора есть только `Inter_1` и `RobotoMono_wght__2`. Поэтому после подключения настоящего X5 Sans сайт начнёт расходиться с текущими рендерами, и пиксельная сверка текста перестанет быть мерилом, пока дизайнер не переэкспортирует макеты со шрифтом. Порядок: получить шрифт → подключить → запросить свежие рендеры → пересверить
